...
# Actor-Critic 3-laryers Network
class GameACSimpleNetwork(GameACNetwork):
  def __init__(self,
               action_size,
               thread_index, # -1 for global
               device="/cpu:0"):
    ...
    with tf.device(self._device), tf.variable_scope(scope_name) as scope:
      hidden_uints_1 = 128
      self.W_fc1, self.b_fc1 = self._fc_variable([STATE_SIZE, hidden_uints_1])

      hidden_uints_2 = 64
      self.W_fc4, self.b_fc4 = self._fc_variable([hidden_uints_1, hidden_uints_2])

      hidden_uints_last = 32
      self.W_fc5, self.b_fc5 = self._fc_variable([hidden_uints_2, hidden_uints_last])

      # weight for policy output layer
      self.W_fc2, self.b_fc2 = self._fc_variable([hidden_uints_last, action_size])

      # weight for value output layer
      self.W_fc3, self.b_fc3 = self._fc_variable([hidden_uints_last, 1])

      # state (input)
      self.s = tf.placeholder("float", [None, STATE_SIZE])

      tmp_out = tf.nn.relu(tf.matmul(self.s, self.W_fc1) + self.b_fc1)

      tmp_out = tf.nn.relu(tf.matmul(tmp_out, self.W_fc4) + self.b_fc4)
      tmp_out = tf.nn.relu(tf.matmul(tmp_out, self.W_fc5) + self.b_fc5)

      # policy (output)
      self.pi = tf.nn.softmax(tf.matmul(tmp_out, self.W_fc2) + self.b_fc2)
      # value (output)
      v_ = tf.matmul(tmp_out, self.W_fc3) + self.b_fc3
